...
public class UrlConnectionDownloader implements Downloader {
    ...
    protected HttpURLConnection openConnection(final Uri path) throws IOException {
    ...
    public Response load(final Uri uri, final boolean localCacheOnly) throws IOException {
        ...
        final boolean fromCache = parseResponseSourceHeader(connection.getHeaderField(RESPONSE_SOURCE));
    ...
    private static class ResponseCacheIcs {
        static Object install(final Context context) throws IOException {
            final File cacheDir = Utils.createDefaultCacheDir(context);
            HttpResponseCache cache = HttpResponseCache.getInstalled();
            if (cache == null) {
                final long maxSize = Utils.calculateDiskCacheSize(cacheDir);
