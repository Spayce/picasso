...
final class Utils {
    ...
    */
    ...
    static String createKey(final Request data) {
        final String result = createKey(data, MAIN_THREAD_KEY_BUILDER);
        MAIN_THREAD_KEY_BUILDER.setLength(0);
        return result;
    }

    static String createKey(final Request data, final StringBuilder builder) {
        if (data.uri != null) {
            final String path = data.uri.toString();
    ...
    static void closeQuietly(final InputStream is) {
        if (is == null) return;
        try {
            is.close();
     ...
     */
    static boolean parseResponseSourceHeader(final String header) {
        if (header == null) {
            return false;
        }
    ...
    static boolean isWebPFile(final InputStream stream) throws IOException {
        final byte[] fileHeaderBytes = new byte[WEBP_FILE_HEADER_SIZE];
    ...
    static int getResourceId(final Resources resources, final Request data) throws FileNotFoundException {
        if (data.resourceId != 0 || data.uri == null) {
            return data.resourceId;
        }

        final String pkg = data.uri.getAuthority();
        if (pkg == null) throw new FileNotFoundException("No package provided: " + data.uri);

        final int id;
        final List<String> segments = data.uri.getPathSegments();
    ...
    static Resources getResources(final Context context, final Request data) throws FileNotFoundException {
        if (data.resourceId != 0 || data.uri == null) {
    ...
    private static class PicassoThread extends Thread {
        public PicassoThread(final Runnable r) {
            super(r);
        }
