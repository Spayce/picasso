...
public class Picasso {
        ...
        public void handleMessage(final Message msg) {
            switch (msg.what) {
                case HUNTER_BATCH_COMPLETE: {
                    @SuppressWarnings("unchecked") final List<BitmapHunter> batch = (List<BitmapHunter>) msg.obj;
                    //noinspection ForLoopReplaceableByForEach
                    for (int i = 0; final int n = batch.size();i < n; i++) {
                        final BitmapHunter hunter = batch.get(i);
                        hunter.picasso.complete(hunter);
                    }
                    break;
                }
                case REQUEST_GCED: {
                    final Action action = (Action) msg.obj;
    ...
    Picasso(final Context context, final Dispatcher dispatcher, final Cache cache, final Listener listener,
            final RequestTransformer requestTransformer, final Stats stats, final boolean debugging) {
    ...
    public void shutdown() {
        ...
        stats.shutdown();
        dispatcher.shutdown();
        for (final DeferredRequestCreator deferredRequestCreator : targetToDeferredRequestCreator.values()) {
    ...
    Request transformRequest(final Request request) {
        final Request transformed = requestTransformer.transformRequest(request);
        if (transformed == null) {
    ...
    void defer(final ImageView view, final DeferredRequestCreator request) {
    ...
    Bitmap quickMemoryCacheCheck(final String key) {
    ...
    void complete(final BitmapHunter hunter) {
        final List<Action> joined = hunter.getActions();
        if (joined.isEmpty()) {
            return;
        }

        final Uri uri = hunter.getData().uri;
        final Exception exception = hunter.getException();
        final Bitmap result = hunter.getResult();
        final LoadedFrom from = hunter.getLoadedFrom();

        //noinspection ForLoopReplaceableByForEach
        for (int i = 0; final int n = joined.size();i < n; i++) {
            final Action join = joined.get(i);
            if (join.isCancelled()) {
                continue;
            }
    ...
    private void cancelExistingRequest(final Object target) {
        final Action action = targetToAction.remove(target);
        if (action != null) {
            action.cancel();
            dispatcher.dispatchCancel(action);
        }
        if (target instanceof ImageView) {
            final ImageView targetImageView = (ImageView) target;
            final DeferredRequestCreator deferredRequestCreator =
    ...
    private static class CleanupThread extends Thread {
        ...
        CleanupThread(final ReferenceQueue<?> referenceQueue, final Handler handler) {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    final RequestWeakReference<?> remove = (RequestWeakReference<?>) referenceQueue.remove();
    ...
    public static class Builder {
         ...
         */
        public Builder memoryCache(final Cache memoryCache) {
            if (memoryCache == null) {
         ...
         */
        public Builder debugging(final boolean debugging) {
            this.debugging = debugging;
            return this;
        }
        ...
        public Picasso build() {
            ...
            final Dispatcher dispatcher = new Dispatcher(context, service, HANDLER, downloader, cache, stats);
