...
public class RequestCreator {
    ...
    RequestCreator(final Picasso picasso, final Uri uri, final int resourceId) {
        if (picasso.shutdown) {
     ...
     */
    public RequestCreator resizeDimen(final int targetWidthResId, final int targetHeightResId) {
        final Resources resources = picasso.context.getResources();
        final int targetWidth = resources.getDimensionPixelSize(targetWidthResId);
        final int targetHeight = resources.getDimensionPixelSize(targetHeightResId);
     ...
     */
    public RequestCreator resize(final int targetWidth, final int targetHeight) {
     ...
     */
    public RequestCreator rotate(final float degrees) {
        data.rotate(degrees);
        return this;
    }
     ...
     */
    public RequestCreator rotate(final float degrees, final float pivotX, final float pivotY) {
     ...
     */
    public RequestCreator config(final Bitmap.Config config) {
        data.config(config);
        return this;
    }
    ...
    public Bitmap get() throws IOException {
        ...
        final Request finalData = picasso.transformRequest(data.build());
        final String key = createKey(finalData, new StringBuilder());

        final Action action = new GetAction(picasso, finalData, skipMemoryCache, key);
    ...
    public void fetch() {
        ...
        if (data.hasImage()) {
            final Request finalData = picasso.transformRequest(data.build());
            final String key = createKey(finalData);

            final Action action = new FetchAction(picasso, finalData, skipMemoryCache, key);
    ...
    public void into(final Target target) {
        ...
        final Request finalData = picasso.transformRequest(data.build());
        final String requestKey = createKey(finalData);

        if (!skipMemoryCache) {
            final Bitmap bitmap = picasso.quickMemoryCacheCheck(requestKey);
            if (bitmap != null) {
                picasso.cancelRequest(target);
        ...
        final Action action = new TargetAction(picasso, target, finalData, skipMemoryCache, requestKey);
        picasso.enqueueAndSubmit(action);
    }
     ...
     */
    public void into(final ImageView target, final Callback callback) {
        if (target == null) {
            throw new IllegalArgumentException("Target must not be null.");
        }
        ...
        if (deferred) {
            ...
            final int measuredWidth = target.getMeasuredWidth();
