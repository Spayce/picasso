...
abstract class BitmapHunter implements Runnable {
    ...
    BitmapHunter(final Picasso picasso, final Dispatcher dispatcher, final Cache cache, final Stats stats, final Action action) {
    ...
    protected void setExifRotation(final int exifRotation) {
    ...
    public void run() {
        ...
        } catch (final Downloader.ResponseException e) {
            exception = e;
            dispatcher.dispatchFailed(this);
    ...
    static void updateThreadName(final Request data) {
        final String name = data.getName();

        final StringBuilder builder = NAME_BUILDER.get();
    ...
    static BitmapHunter forRequest(final Context context, final Picasso picasso, final Dispatcher dispatcher,
                                   final Cache cache, final Stats stats, final Action action, final Downloader downloader) {
        if (action.getData().resourceId != 0) {
            return new ResourceBitmapHunter(context, picasso, dispatcher, cache, stats, action);
        }
        final Uri uri = action.getData().uri;
    ...
    static BitmapFactory.Options createBitmapOptions(final Request data) {
    ...
    static void calculateInSampleSize(final int reqWidth, final int reqHeight, final BitmapFactory.Options options) {
        calculateInSampleSize(reqWidth, reqHeight, options.outWidth, options.outHeight, options);
    }

    static void calculateInSampleSize(final int reqWidth, final int reqHeight, final int width, final int height,
    ...
    static Bitmap transformResult(final Request data, Bitmap result, final int exifRotation) {
        final int inWidth = result.getWidth();
        final int inHeight = result.getHeight();
        ...
        if (data.needsMatrixTransform()) {
            ...
            final int targetHeight = data.targetHeight;

            final float targetRotation = data.rotationDegrees;
            if (targetRotation != 0) {
